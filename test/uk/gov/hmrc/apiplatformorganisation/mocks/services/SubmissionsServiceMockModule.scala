...
trait SubmissionsServiceMockModule extends MockitoSugar with ArgumentMatchersSugar {
  ...
  protected trait BaseSubmissionsServiceMock {
    ...
    object Submit {
      ...
      def thenReturn(submission: Submission) =
        when(aMock.submit(*[SubmissionId], *)).thenReturn(successful(Right(submission)))
    ...
    object Decline {
      ...
      def thenReturn(submission: Submission) =
        when(aMock.decline(*[SubmissionId], *, *)).thenReturn(successful(Right(submission)))

      def thenFails(error: String) =
        when(aMock.decline(*[SubmissionId], *, *)).thenReturn(successful(Left(error)))
    }
