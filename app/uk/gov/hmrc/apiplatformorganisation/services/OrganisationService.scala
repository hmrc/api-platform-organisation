...
class OrganisationService @Inject() (
  ...
  def create(organisationName: OrganisationName, organisationType: Organisation.OrganisationType, requestedBy: UserId)(implicit ec: ExecutionContext): Future[Organisation] = {
    organisationRepository.save(StoredOrganisation.create(organisationName, organisationType, requestedBy, instant)).map(StoredOrganisation.asOrganisation(_))
  }
  ...
  def addCollaborator(organisationId: OrganisationId, email: LaxEmailAddress, role: Collaborator.Role)(implicit ec: ExecutionContext, hc: HeaderCarrier)
    ...
    (
      for {
        ...
        existingUserDetails <- liftF(thirdPartyDeveloperConnector.getRegisteredOrUnregisteredUsers(getAdminAndRiUserIds(organisation)))
  ...
  private def getAdminAndRiUserIds(organisation: StoredOrganisation): List[UserId] = {
    organisation.collaborators.filter(c => c.isAdministrator || c.isResponsibleIndividual).map(_.userId).toList
  }
  ...
  def removeCollaborator(organisationId: OrganisationId, userId: UserId, email: LaxEmailAddress)(implicit ec: ExecutionContext, hc: HeaderCarrier)
    ...
    (
      for {
        ...
        userDetails         <- liftF(thirdPartyDeveloperConnector.getRegisteredOrUnregisteredUsers(getAdminAndRiUserIds(organisation)))
